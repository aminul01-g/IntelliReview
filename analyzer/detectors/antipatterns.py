...
class AntiPatternDetector(Detector):
    ...
    def detect(self, code: str, filename: str = "unknown", language: str = "python", **kwargs) -> List[Dict]:
        ...
        # This detector requires an AST. If not provided in kwargs, we try to generate a basic one
        # or skip AST-dependent checks.
        ast = kwargs.get("ast")
        if not ast:
            # Very simplified fallback: don't run AST checks if no AST is provided
            # In the full pipeline, the parser provides the AST.
            pass
    ...
    def _detect_python_antipatterns(self, code: str, ast: Optional[ParsedNode]) -> List[Dict]:
        ...
        # Magic Numbers (Regex based, doesn't need AST)
        ...
        # Asyncio blocks in loops (Regex based)
        in_loop = False
        in_loop_indent = 0
        for i, line in enumerate(lines, 1):
